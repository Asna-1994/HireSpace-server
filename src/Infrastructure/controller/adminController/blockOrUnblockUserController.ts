...
export class BlockOrUnblockUserController {
  ...
  async blockOrUnblock(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if ( !userId || !action) {
throw new CustomError(STATUS_CODES.BAD_REQUEST,"Missing required query parameters:userId, or action")
}

    const updatedUSer = await this.blockOrUnblockUseCase.execute(userId as string, action as string)
