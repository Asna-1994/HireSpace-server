...
export async function createConnectionRequest(req: Request, res: Response, next : NextFunction): Promise<void> {
  try {
    ...
    const newRequest = await connectionRequestUseCase.createConnectionRequest(fromUser, toUser);
...
export async function getAllConnectionRequestsForUser(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const {pendingRequests,  totalRequests,} = await connectionRequestUseCase.getConnectionRequestForUser(userId, page, limit);
...
export async function getAllConnectionRequestsSentByUser(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const { pendingRequests, totalRequests } = await connectionRequestUseCase.getConnectionRequestSentByUser(userId, page, limit);
      res.status(STATUS_CODES.SUCCESS).json({
        success : true,
        message:MESSAGES.DATA_FETCHED,
        data: {
...
export async function getAllConnectionsByUserId(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { userId } = req.params;
      ...
      const {connections,total} = await connectionRequestUseCase.findAllConnections(userId,page, limit, search);
  ...
  export async function getRecommendationsForUser(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { userId } = req.params;
      const page = parseInt(req.query.page as string) || 1;
      const limit = parseInt(req.query.limit as string) || 10;
  
      const {users, total}= await connectionRequestUseCase.recommendationForUsers(userId, page, limit)
...
export async function acceptConnectionRequest(req: Request, res: Response,next : NextFunction): Promise<void> {
  try {
    const { id } = req.params;
    const acceptedRequest = await connectionRequestUseCase.acceptConnectionRequest(id);
    res.status(STATUS_CODES.SUCCESS).json({
        success : true,
...
export async function rejectConnectionRequest(req: Request, res: Response, next : NextFunction): Promise<void> {
  try {
    const { id } = req.params;
    const rejectedRequest = await connectionRequestUseCase.rejectConnectionRequest(id);
    res.status(STATUS_CODES.SUCCESS).json({
        success : true,
