...
export class UserJobApplicationController {
  ...
  async applyForJob(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        if(!userId || !jobPostId || !companyId){
            throw new CustomError(STATUS_CODES.BAD_REQUEST, "please provide the necessary parameters")
        }

      const newApplication = await this.userJobApplicationUseCase.createJobApplication({userId, jobPostId, coverLetter, companyId})
  ...
  async getAllJobApplicationForUser(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        const { allApplications, totalApplications } = await this.userJobApplicationUseCase.getAllApplicationForUser(
            userId,
            Number(page),
            Number(limit),
