...
export class AddMembersController {
  ...
  async addMember(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const {updatedUser, updatedCompany} = await this.addMembersUseCase.execute(userEmail, companyId, userRole)
  ...
  async removeMember(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const {updatedUser, updatedCompany} = await this.addMembersUseCase.removeMember( companyId,userId)
