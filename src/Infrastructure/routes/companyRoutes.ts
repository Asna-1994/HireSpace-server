...
import { checkIfAUthenticated, isCompany } from '../middleware/authenticationMiddleware';
...
const verifyOtpUseCase = new VerifyOtpUseCase(tempCompanyRepository,companyRepository, userRepository,companyProfileRepository);
const verifyOtpController = new VerifyOtpController(verifyOtpUseCase);

const signupUseCase = new SignupUseCase(tempCompanyRepository, companyRepository,userRepository, sendOtpEmail);
...
const resendOtpUseCase = new ResendOtpUseCase(tempCompanyRepository, sendOtpEmail);
...
const uploadLogoController = new UploadLogoController(fileUploadUseCase,companyRepository)
const uploadDocumentController = new UploadDocumentController(fileUploadUseCase,companyRepository)


const addMembersUseCase = new AddMembersUseCase(companyRepository, userRepository)
const addMemberController = new AddMembersController(addMembersUseCase)

const manageProfileUseCase = new ManageProfileUseCase(companyRepository, companyProfileRepository)
...
const companyJobApplicationUseCase = new CompanyJobApplicationUseCase(jobApplicationRepository)
const companyJobApplicationController = new CompanyJobApplicationController(companyJobApplicationUseCase)

const router = Router();

router.post('/verify-otp', (req, res, next) => verifyOtpController.verifyOtp(req, res, next));
 router.post("/signup",companySignupValidationRules,validateRequest,  (req: Request, res: Response, next: NextFunction) => signupController.signup(req, res, next));
...
router.post('/resend-otp', (req, res, next) => resendOtpController.resendOtp(req, res, next));
 ...
 router.patch('/upload-company-logo/:companyId',checkIfAUthenticated ,isCompany, upload.single("companyLogo"), (req, res, next) =>uploadLogoController.uploadLogo(req, res, next));
 router.patch('/delete-logo/:companyId',checkIfAUthenticated ,isCompany,  (req, res, next) =>uploadLogoController.deleteLogo(req, res, next));
 router.patch('/upload-document/:companyId',checkIfAUthenticated ,isCompany,  upload.single("verificationDocument"), (req, res, next) =>uploadDocumentController.uploadDocument(req, res, next));
 router.patch('/delete-document/:companyId',checkIfAUthenticated ,isCompany,  (req, res, next) =>uploadDocumentController.deleteDocument(req, res, next));
 router.patch('/:companyId/add-member',checkIfAUthenticated ,isCompany, (req, res, next) =>addMemberController.addMember(req, res, next));
 router.delete('/:companyId/remove-member/:userId',checkIfAUthenticated, isCompany, (req, res, next) =>addMemberController.removeMember(req, res, next));
 router.get('/:companyId/all-members',checkIfAUthenticated,isCompany,  (req, res, next) =>addMemberController.getAllMembers(req, res, next));
 router.patch('/update-basic-detail/:companyId',checkIfAUthenticated, isCompany, companyEditValidationRules,validateRequest,  (req: Request, res: Response, next: NextFunction) =>manageProfileController.updatedCompanyDetails(req, res, next));
 router.get('/company-profile-details/:companyId',checkIfAUthenticated, isCompany,  (req, res, next) =>manageProfileController.getCompanyProfile(req, res, next));
 router.patch('/complete-profile/:companyId',checkIfAUthenticated, isCompany,  (req, res, next) =>manageProfileController.updatedCompanyProfile(req, res, next));
//job posts
router.post('/job-post/:companyId/:userId',checkIfAUthenticated ,isCompany,  (req, res, next) => jobPostController.createJobPost(req, res, next));
router.get('/all-job-posts/:companyId',checkIfAUthenticated ,isCompany,  (req, res, next) => jobPostController.getAllJobPost(req, res, next));


 //job applications
 router.patch('/job-application-status/:applicationId',checkIfAUthenticated , isCompany, (req, res, next) => companyJobApplicationController.updateApplicationStatus(req, res, next));
router.get('/all-applications/:companyId',checkIfAUthenticated ,isCompany,  (req, res, next) => companyJobApplicationController.getAllJobApplicationForCompany(req, res, next));
