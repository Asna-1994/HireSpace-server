...
import { checkIfAUthenticated, isAdmin } from '../middleware/authenticationMiddleware';
...
const getAllUsersUseCase = new GetAllUsersUseCase(userRepository, spamRepository);
const getAllUsersController = new GetAllUsersController(getAllUsersUseCase);

const getAllCompanyUseCase = new GetAllCompaniesUseCase(companyRepository);
const getAllCompanyController = new GetAllCompanyController(getAllCompanyUseCase);

const blockOrUnblockUserUseCase = new BlockOrUnblockUserUseCase(userRepository);
const blockOrUnblockUserController = new BlockOrUnblockUserController(blockOrUnblockUserUseCase);

const blockOrUnblockCompanyUseCase = new BlockOrUnblockCompanyUseCase(companyRepository);
const blockOrUnblockCompanyController = new BlockOrUnblockCompanyController(blockOrUnblockCompanyUseCase);

const dashboardUseCase = new DashboardUseCase(userRepository,companyRepository, spamRepository, jobPostRepository, jobApplicationRepository);
...
router.get('/all-users',checkIfAUthenticated ,(req, res, next) => getAllUsersController.getAllUsers(req, res, next));
router.get('/all-companies',checkIfAUthenticated , (req, res, next) => getAllCompanyController.getAllCompanies(req, res, next));
router.patch('/block-or-unblock-user/:userId/:action',checkIfAUthenticated,isAdmin , (req, res, next) => blockOrUnblockUserController.blockOrUnblock(req, res, next));
router.patch('/block-or-unblock-company/:companyId/:action',checkIfAUthenticated ,isAdmin, (req, res, next) => blockOrUnblockCompanyController.blockOrUnblock(req, res, next));
router.patch('/:companyId/verify-company',checkIfAUthenticated ,isAdmin, (req, res, next) => blockOrUnblockCompanyController.verifyCompany(req, res, next));
router.get('/premium-users',checkIfAUthenticated ,isAdmin, (req, res, next) => getAllUsersController.getPremiumUsers(req, res, next));
router.get('/spam-reports',checkIfAUthenticated ,isAdmin, (req, res, next) => getAllUsersController.getSpamReports(req, res, next));

router.get('/dashboard-stats',checkIfAUthenticated ,isAdmin, (req, res, next) => dashboardController.getDashboardStats(req, res, next));
