...
export const checkIfAUthenticated = async(req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!token) {
        throw new CustomError(STATUS_CODES.UNAUTHORIZED,"You are not authorised, Please login")
    ...
    if(req.user?.entity === 'user'){
      ...
      if(user?.isBlocked){
        throw new CustomError(STATUS_CODES.FORBIDDEN,"Account is blocked, Please contact admin")
      }
    }
