...
export interface CompanyRepository {
  ...
  delete(email: string): Promise<void>;
   findCompanies(offset: number, limit: number,  filter: object): Promise<{ companies: Company[], total: number }> 
  create(company: Partial<Company>): Promise<Company>;
     findCompaniesWithPagination(offset: number, limit: number,filter : object): Promise<Company[]> 
