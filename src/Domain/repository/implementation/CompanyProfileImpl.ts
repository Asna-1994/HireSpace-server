...
import { CompanyProfile, normalizeCompanyProfile } from "../../entities/CompanyProfile";
...
export class CompanyProfileImpl implements CompanyProfileRepository {
  ...
  async findOneAndUpdate(filter: object, update: object, options: object): Promise<CompanyProfile | null> {
    const companyProfile = await CompanyProfileModel.findOneAndUpdate(filter, update, options).lean();
    return companyProfile ? normalizeCompanyProfile(companyProfile) : null;
  }

  async create(companyProfile: Partial<CompanyProfile>): Promise<CompanyProfile> {
