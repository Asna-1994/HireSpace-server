...
import { JobApplication, normalizeJobApplication} from "../../entities/JobApplication";
...
export class JobApplicationRepositoryImpl implements JobApplicationRepository {
  ...
  async findApplicationWithPagination(offset: number, limit: number, filter: object): Promise<{ jobApplications: JobApplication[]; totalApplications: number }> {
    try {
        ...
        const totalApplications = await JobApplicationModel.countDocuments(filter);

        console.log("Job applications in repo", jobApplications);
        return { jobApplications: jobApplications as JobApplication[], totalApplications };
    } catch (err) {
  ...
  async findById(jobApplicationId: string): Promise<JobApplication | null> {
    const jobApplication = await JobApplicationModel.findById(jobApplicationId).lean();
    return jobApplication? jobApplication : null;
  }

  async update(jobApplication: JobApplication): Promise<JobApplication> {
    const updatedApplication = await JobApplicationModel.findByIdAndUpdate( jobApplication._id,jobApplication,{ new: true } )
      .lean()
      .exec();
  ...
  async findOneAndUpdate(
    ...
    const jobApplication = await JobApplicationModel.findOneAndUpdate(filter, update, options).lean();
  ...
  async findByIdAndUpdate(
    ...
    const jobApplication = await JobApplicationModel.findByIdAndUpdate(jobApplicationId, update, options).lean();
    return jobApplication 
  }


    async create(jobApplication:  Partial<JobApplication>): Promise<JobApplication> {
    ...
    async count(filter: Record<string, any>): Promise<number> {
      try {
        ...
        console.error("Error counting documents in jobApplicationRepository:", error);
        throw new CustomError(STATUS_CODES.INTERNAL_SERVER_ERROR, "Error while counting job applications");
      }
    }
  ...
  async getStatusDistribution(query: any): Promise<Array<{ name: string; value: number }>> {
