...
export class PlanRepositoryImpl implements PlanRepository {
  ...
  async updatePlan(id: string, planData: Partial<Plans>): Promise<Plans | null> {
   const plan  = await PlanModel.findByIdAndUpdate(id, planData, { new: true }).lean().exec();
   return plan ? normalizePlans(plan) : null
  }


  async deletePlan(planId: string): Promise<Plans | null> {
    const plan  = await PlanModel.findByIdAndUpdate(planId, {isDeleted: true}).lean().exec();
    return plan ? normalizePlans(plan) : null
   }
 

  async getAllPlans(filter : object, skip? :number,limit? : number): Promise<Plans[]> {
    let plans;
