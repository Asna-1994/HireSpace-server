...
try{
    ...
    const jobSeekerProfile = await this.jobSeekerProfileRepository.findOne({userId : userId})
    if(!jobSeekerProfile){
...
return jobSeekerProfile.education
  ...
  if (err instanceof CustomError) {
    ...
    throw new CustomError(STATUS_CODES.INTERNAL_SERVER_ERROR,"Error in getting all education details")
}
  }
    ...
    try{
        ...
        if(jobSeekerProfile.education){
           educationIndex = jobSeekerProfile?.education.findIndex(
            ...
            if (educationIndex === -1) {
                throw new CustomError(STATUS_CODES.NOT_FOUND, "Education with ID not found");
            }
    
            jobSeekerProfile.education.splice(educationIndex as number, 1);
    
        }
        let updatedProfile = await this.jobSeekerProfileRepository.update(jobSeekerProfile)
      ...
      if (err instanceof CustomError) {
        ...
        throw new CustomError(STATUS_CODES.INTERNAL_SERVER_ERROR,"Error in deleting education details")
      ...
      async addExperience(experienceData: {
        ...
        if (!updatedProfile) {
          throw new CustomError(
            STATUS_CODES.INTERNAL_SERVER_ERROR,
            experienceId ? "Failed to update experience" : "Failed to add experience"
          );
        }
...
async getAllExperience( userId: string) {
    ...
    return jobSeekerProfile.workExperience
        ...
        throw new CustomError(STATUS_CODES.INTERNAL_SERVER_ERROR,"Error in getting all experience details")
    }
      }
      ...
      async deleteExperienceById(userId : string, experienceId : string) {
        try{
          ...
          if (err instanceof CustomError) {
            ...
            throw new CustomError(STATUS_CODES.INTERNAL_SERVER_ERROR,"Error in deleting experience details")
        ...
        try{
            ...
            const updatedProfile = await this.jobSeekerProfileRepository.findOneAndUpdate(
                { userId }, 
        ...
        return jobSeekerProfile.skills
            ...
            throw new CustomError(STATUS_CODES.INTERNAL_SERVER_ERROR,"Error in getting all skill details")
        }
          }
          ...
          async deleteSkill(userId: string, skillName: string) {
            try {
              ...
              if (jobSeekerProfile.skills) {
                ...
                if (skillName === 'technicalskills' && jobSeekerProfile.skills.technicalSkills) {
              ...
              throw new CustomError(STATUS_CODES.INTERNAL_SERVER_ERROR, "Error in deleting skill");
            }
          }
        ...
        certificateUrl
            ...
            if (!updatedProfile) {
              throw new CustomError(
                STATUS_CODES.INTERNAL_SERVER_ERROR,
              certificateId ? "Failed to update certificate" : "Failed to add certificate"
              );
            }
          ...
          async getAllCertificates( userId: string) {
            ...
            return jobSeekerProfile.certificates
                ...
                throw new CustomError(STATUS_CODES.INTERNAL_SERVER_ERROR,"Error in getting all certificate details")
            }
              }
                ...
                try{
                    ...
                    if(jobSeekerProfile.certificates){
                       certificateIndex = jobSeekerProfile?.certificates.findIndex(
                        ...
                        if (certificateIndex === -1) {
                            throw new CustomError(STATUS_CODES.NOT_FOUND, "Certificate with ID not found");
                        }
                  ...
                  async getJobSeekerProfile( userId: string) {
                    ...
                    return jobSeekerProfile
                        ...
                        throw new CustomError(STATUS_CODES.INTERNAL_SERVER_ERROR,"Error in getting profile")
                    }
                  ...
                  async saveJobs( userId: string ,  jobPostId  :string) {
                    try{
                        ...
                        if (user.savedJobs.includes(jobPostIdObject)) {
                          throw new CustomError(STATUS_CODES.BAD_REQUEST, "Job post already saved");
                      }
