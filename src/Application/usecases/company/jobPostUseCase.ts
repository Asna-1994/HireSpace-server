...
try{
    const allJobPosts = await this.jobPostRepository.find({companyId : companyId, isDeleted : false});
    return allJobPosts;
}
catch(err){
  if (err instanceof CustomError) {
    ...
    throw new CustomError(STATUS_CODES.INTERNAL_SERVER_ERROR, "Error while getting job posts by company Id")
}
  ...
  async deleteJobPostById(jobPostId: string ) {
    ...
    try{
        const deletedPost = await this.jobPostRepository.findByIdAndUpdate(jobPostId,{isDeleted : true},{new : true})
        return deletedPost;
    }
    catch(err){
      if (err instanceof CustomError) {
        ...
        throw new CustomError(STATUS_CODES.INTERNAL_SERVER_ERROR, "Error while deleting job posts ")
    }
