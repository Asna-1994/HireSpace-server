...
export class ResendOtpUseCase {
  ...
  async execute(email: string): Promise<TempCompany> {
    ...
    if (!existingTempCompany) {
      throw new CustomError(STATUS_CODES.NOT_FOUND, "No Company fround  to send otp");
    }
...
const updatedExistingTempCompany= await this.tempCompanyRepository.updateOne(existingTempCompany)

if (!updatedExistingTempCompany) {
    throw new CustomError(STATUS_CODES.INTERNAL_SERVER_ERROR, "Failed to update Company with new OTP");
  }
