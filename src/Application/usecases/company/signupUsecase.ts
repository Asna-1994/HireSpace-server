...
export class SignupUseCase {
  ...
  async execute(companyData: {
    ...
    if (existingCompany) {
      throw new CustomError(STATUS_CODES.BAD_REQUEST, "Company already registered");
    }
    const existingTempCompany = await this.tempCompanyRepository.findByEmail(email);
    if (existingTempCompany) {
     await this.tempCompanyRepository.deleteByEmail(email)
    }
const companyAdmin = await this.userRepository.findByEmail(companyAdminEmail)
if(!companyAdmin){
  throw new CustomError(STATUS_CODES.NOT_FOUND, "No user found with this email");
}
if(companyAdmin.userRole !== 'companyAdmin' ){
  throw new CustomError(STATUS_CODES.BAD_REQUEST, "User not registered as a company admin");
}
    ...
    const savedTempCompany = await this.tempCompanyRepository.create(tempCompany);
