...
export class LoginUseCase {
    ...
    async execute(loginData: { email: string; password: string }) {
      ...
      if (!email || !password) {
        throw new CustomError(STATUS_CODES.BAD_REQUEST, "Please provide Email and password");
      }
      ...
      if (existingUser) {
        ...
        const isUserPasswordValid = await comparePassword(password, existingUser.password as string);
        ...
        ? await this.companyRepository.findById(existingUser.companyId.toString()) 
        : null;
      ...
      const isCompanyPasswordValid = await comparePassword(password, existingCompany.password as string);
      if (!isCompanyPasswordValid) {
        throw new CustomError(STATUS_CODES.BAD_REQUEST, MESSAGES.INVALID_PASSWORD);
      }
