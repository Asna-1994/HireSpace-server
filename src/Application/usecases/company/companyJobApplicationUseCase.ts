...
export class CompanyJobApplicationUseCase {
    ...
    async getAllApplicationForCompany({
      ...
      try {
        ...
        const { jobApplications, totalApplications } = await this.jobApplicationRepository.findApplicationWithPagination(offset, limit, filter);
        ...
        throw new CustomError(STATUS_CODES.INTERNAL_SERVER_ERROR, "Error while getting applications");
      }
    }
      ...
      async updateStatus(applicationId:string, status :  "pending" | "reviewed" | "accepted" | "rejected"){
try{
const application = await this.jobApplicationRepository.findById(applicationId)
if(!application){
  throw new CustomError(STATUS_CODES.NOT_FOUND,"No application found with this ID")
}

if (!application._id || !application.userId || !application.jobPostId) {
  throw new CustomError(STATUS_CODES.INTERNAL_SERVER_ERROR, "Application data is incomplete.");
}
...
return updatedApplication
  ...
  throw new CustomError(STATUS_CODES.INTERNAL_SERVER_ERROR,"Error while updating status")
}
      }
  }
