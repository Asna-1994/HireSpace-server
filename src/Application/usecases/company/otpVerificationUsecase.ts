...
export class VerifyOtpUseCase {
  ...
  async execute(email: string, otp: string): Promise<Company> {
    ...
    console.log("OTP Expiry:", tempCompany.otpExpiry, "Current Time:", Date.now());
    ...
    const adminUser = await this.userRepository.findByEmail(tempCompany.companyAdminEmail);
    ...
    const companyProfile = await this.companyProfileRepo.create({companyId:new mongoose.Types.ObjectId(createdCompany._id) })
